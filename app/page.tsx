    ...
    >
      ...
      <div className="relative flex items-center justify-between gap-2">
        ...
        {status === 'completed' && output?.confidence && (
          <ConfidenceBadge level={output.confidence} isDark={isDark} />
        )}
        {status === 'failed' && (
          <span className="text-[9px] font-mono px-1.5 py-0.5 rounded"
...
export default function VeracityDashboard() {
  ...
  const handleNewQuery  = () => { setMessages([]); setFollowUps([]); setExpandedDomain(null); setAttachedImages([]); };
        ...
        <div className="px-3 pb-3">
          <div className="rounded-lg overflow-hidden" style={{ border: `1px solid ${borderC}`, background: isDark ? '#0a0a0a' : '#f9f9f9' }}>
            ...
            <div className="py-1 px-1">
              {ALL_DOMAINS.map(d => (
                <SidebarAgentRow key={d} domain={d} run={getRunForDomain(d)} isDark={isDark} />
              ))}
            </div>
          </div>
        </div>
          ...
          {recentQueries.length > 0 && (
            <div className="mb-3">
              ...
              {recentQueries.slice().reverse().slice(0, 5).map((q, i) => (
                <button key={i} onClick={() => handleSend(q)} title={q}
              ...
              <div className="rounded-lg overflow-hidden" style={{
                ...
                <div className="flex items-center justify-between px-5 py-3.5"
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <ConfidenceBadge level={expandedOutput.confidence} isDark={isDark} />
                  </div>
                        ...
                        {currentResult.recommendations.map((rec: any, i: number) => (
                          <div key={i} className="rounded-lg p-4 flex flex-col gap-2.5"
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              ...
                              <ConfidenceBadge level={rec.confidence ?? (rec.score >= 80 ? 'high' : rec.score >= 55 ? 'medium' : 'low')} isDark={isDark} />
                            </div>
