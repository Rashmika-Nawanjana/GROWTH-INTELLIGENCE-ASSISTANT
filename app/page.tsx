...
import {
  Send, Plus, MessageSquare, Search, ChevronRight, Check, RefreshCw,
  ArrowUpRight, Clock, ShieldCheck, Database, LogOut, User, Trash2,
} from 'lucide-react';
...
import {
  ...
  saveMessage, loadMessages, updateSessionTitle,
...
// Convert StoredMessage → Message (re-hydrate metadata fields)
...
export default function VeracityChat() {
  ...
  // ── New query — clear messages, session created on first send ────────────────
  ...
  const handleDeleteSession = async (e: React.MouseEvent, sessionId: string) => {
    ...
    if (currentSessionId === sessionId) handleNewQuery();
  ...
  const handleSend = async (text: string) => {
    ...
    if (!sessionId) {
      // First message in a new session — create the session
    ...
    if (sessionId) {
      ...
      if (finalOutput) {
        ...
        // Update session title if this is the first answer
        ...
        // Extract + update persistent user memory in the background (non-blocking)
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4">
          ...
          {loadingSessions ? (
            <div className="flex flex-col gap-1 px-2">
              {[1,2,3].map(i => (
                <div key={i} className="h-8 bg-border/60 rounded-lg animate-pulse-line" />
              ))}
            </div>
          ) : sessions.length === 0 ? (
            <p className="text-xs text-muted-foreground px-2 py-2">No saved queries yet.</p>
          ) : (
            <div className="flex flex-col gap-0.5">
              {sessions.map(session => (
                <button
                  ...
                  className={`group w-full flex items-center gap-2 px-2 py-2 rounded-lg text-left transition-colors ${
                    currentSessionId === session.id
                      ? 'bg-accent/10 text-accent'
                      : 'hover:bg-border/60 text-foreground'
                  }`}
                >
        ...
        <div className="h-14 border-b border-border bg-white/80 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
          <div className="flex items-center gap-3">
            <span className="font-medium text-sm text-foreground truncate max-w-xs">
              {currentSessionTitle ?? (messages.length > 0 ? 'Intelligence Query' : 'New Intelligence Query')}
            </span>
            <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200 text-xs font-mono flex items-center gap-1.5 shrink-0">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse-dot" />
              Live
            </span>
          </div>
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[60vh] text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <p className="text-muted-foreground text-sm mb-8">
                  Live signals from 16+ sources · 6 intelligence domains · Sourced & confidence-scored
                </p>
                                ...
                                <div key={output.domain} className="rounded-xl border border-border overflow-hidden">
                                  ...
                                  {isExpanded && (
                                    <div className="px-4 py-4 flex flex-col gap-4 bg-white">
                                      <ArtifactRenderer output={output} product={msg.orchestratorOutput!.product} />
